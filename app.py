...
<script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-7263468237820563" crossorigin="anonymous"></script>
<style>
...
.whatsapp-float{position:fixed;right:18px;bottom:18px;width:58px;height:58px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:#25D366;color:#fff;font-size:31px;box-shadow:0 10px 25px rgba(0,0,0,.35);z-index:9999;border:2px solid rgba(255,255,255,.18);transition:.2s}.whatsapp-float:hover{transform:scale(1.08)}
...
<a class="whatsapp-float" href="https://wa.me/263777698250?text=Hello%20RickNet%20Support" target="_blank" rel="noopener" aria-label="WhatsApp Support" title="WhatsApp Support">☏</a>
...
def home():
    ...
    <div class="info-strip">
      <b>🎁 Refer a Friend:</b> <span class="muted">Invite a new member and earn +1 Net Credit when they register.</span>
      <div class="actions"><a class="btn green" href="{{ url_for('profile') }}#referralLink">🔗 Get My Referral Link</a></div>
    </div>
    {% endif %}
...
def register():
    ...
    referral_username = request.form.get("ref", request.args.get("ref", "")).strip()

    if request.method == "POST":
        phone = request.form.get("phone", "").strip()
        username = request.form.get("username", "").strip()
        password = request.form.get("password", "")
        referral_username = request.form.get("ref", referral_username).strip()
        ...
        if len(phone) < 6 or len(username) < 3 or len(password) < 6:
            ...
            if referral_username:
                referrer = User.query.filter_by(username=referral_username).first()
                if referrer and referrer.username.lower() == username.lower():
                    referrer = None
            ...
            db.session.add(user)
            if referrer:
                referrer.net_credits += 1.00
            db.session.commit()
            login_user(user)
            if referrer:
                flash(f"Account created successfully. {referrer.username} earned +1.00 Net Credit for your referral.")
            else:
    ...
    <div class="card" style="max-width:520px;margin:auto">
      ...
      {% if referral_username %}<div class="info-strip"><b>🎁 Referral detected!</b><br><span class="muted">You were invited by @{{ referral_username }}. They will receive +1 Net Credit after you create your account.</span></div>{% endif %}
      <form method="post">
        <input type="hidden" name="ref" value="{{ referral_username }}">
    ...
    """
    return page(body, "Register", referral_username=referral_username)
...
def profile():
    ...
    <div class="card">
      <h2>🎁 Refer & Earn</h2>
      <p class="muted">Invite a new friend to join RickNet. When they create an account through your link, you earn <b>+1 Net Credit</b>.</p>
      <input id="referralLink" value="{{ request.url_root }}register?ref={{ current_user.username|urlencode }}" readonly onclick="this.select()">
      <div class="actions">
        <button class="btn green" type="button" onclick="navigator.clipboard.writeText(document.getElementById('referralLink').value); this.innerText='✅ Link Copied!'">📋 Copy Referral Link</button>
        <a class="btn" target="_blank" rel="noopener" href="https://wa.me/?text={{ ('Join RickNet Free Data using my referral link: ' ~ request.url_root ~ 'register?ref=' ~ current_user.username)|urlencode }}">💬 Share on WhatsApp</a>
      </div>
    </div>
